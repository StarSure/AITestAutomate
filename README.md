...
- 自动生成接口测试用例
- 在平台内审核测试用例状态、负责人和备注
- 将 ready 用例编排进 Smoke / Full Regression / 鉴权防线 等执行计划
- 执行测试并输出报告
- 根据执行结果自动回流缺陷并跟踪分诊/解决状态
- 保存历史执行报告
- 展示失败原因和响应详情
- 使用 SQLite 持久化工作区状态
- 使用 Docker 一键启动
...
- 自动判断接口风险级别
- 高风险接口默认不直接执行
- 支持用例审核状态：`draft / ready / blocked`
- 支持维护用例负责人和审核备注

### 5. 任务中心

- 默认生成 Smoke 回归、Full Regression、鉴权防线 三类计划
- 支持配置计划负责人、触发方式、执行节奏
- 支持将 ready 用例编排进计划
- 支持按计划执行，而不是一次性跑全部用例
...
3. 平台自动识别业务接口和页面元素
4. 勾选确认要纳入回归的接口资产
5. 平台根据确认后的接口生成测试用例，并在测试资产页完成审核
6. 在任务中心把 ready 用例编排进计划
7. 点击运行计划
8. 查看历史报告、通过 / 失败 / 跳过结果、失败原因和响应内容
9. 在缺陷闭环页跟进自动生成的失败项
